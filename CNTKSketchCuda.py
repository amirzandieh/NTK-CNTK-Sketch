...
def krelu(q,h):
    ...
    beta_ = quadprog_solve_qp(np.dot(U, U.T), -np.dot(U,w) , np.concatenate((Z[0:200,:]-Z[1:201,:], -np.eye(q+1)),axis=0), np.zeros(q+201), Z[200,:][np.newaxis,:],y[200])
...
def SRHTCmplx_Stndrd(X, P, D):
    ...
    Y = np.sqrt(1/len(P))* torch.fft.fftn(X * D.unsqueeze(0).unsqueeze(2).unsqueeze(3), dim=1)[:,P,:,:]
        ...
        for j in range(1,len(U)):
            ...
            for i in range(lgq):
                if j%(2**(i+1)) == 0 :
                    ...
                    if i == 0:
                        V[i][p,:,:,:,:] = SRHTCmplx_Stndrd(X, self.Tree_P[h][i][p,0,:], self.Tree_D[h][i][p,0,:]) #TSRHTCmplx(X, E1, self.Tree_P[h][i][p,:,:], self.Tree_D[h][i][p,:,:])
                    else:
                        if (i-1,2*p) in SetE1:
                            V[i][p,:,:,:,:] = V[i-1][2*p+1,:,:,:,:]
                        else:
